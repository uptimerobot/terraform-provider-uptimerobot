...
func validateURL(
	...
	if data.URL.IsNull() {
		if monitorType != MonitorTypeHEARTBEAT {
			resp.Diagnostics.AddAttributeError(
				path.Root("url"),
				"url is required for this monitor type",
				"url must be specified for HTTP, KEYWORD, PING, PORT, DNS, API, and UDP monitors. "+
					"Only HEARTBEAT monitors may omit url (it is generated by UptimeRobot).",
			)
		}
		return
	}
