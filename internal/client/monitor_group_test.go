...
func TestClient_ListAllMonitorGroups_RejectsCursorCycle(t *testing.T) {
	t.Parallel()
	...
	c.httpClient = &http.Client{
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			...
			case "/monitor-groups?cursor=202":
	...
	if strings.Join(calls, ",") != "GET /monitor-groups,GET /monitor-groups?cursor=101,GET /monitor-groups?cursor=202" {
		t.Fatalf("unexpected calls %#v", calls)
	}
}
