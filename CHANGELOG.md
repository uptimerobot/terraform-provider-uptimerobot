...
- Added contributor guidance for keeping public repository activity free of private references and sensitive data.
...
- Added shared API rate-limit handling that honors `Retry-After`/`X-RateLimit-Reset`, delays subsequent requests after quota exhaustion, and retries explicit `429 Too Many Requests` responses for JSON and multipart create/update requests.
...
- Clarified that HEARTBEAT monitor URLs are generated by UptimeRobot and must be omitted from configuration.
- Added import command examples for integration, maintenance window, and public status page resources.
- Clarified that existing resources created in the UptimeRobot UI or API can be imported by numeric resource ID.

## 1.4.4 — 2026-04-29

### Fixed

- Fixed heartbeat monitor planning when `url = ""` is configured by returning a validation error instead of producing an invalid Terraform plan. HEARTBEAT monitor URLs must be omitted because they are generated by UptimeRobot.
